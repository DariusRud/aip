...
export default function DocumentEdit({ documentId, onClose, onSaved }: Props) {
  ...
  const fetchData = async () => {
    try {
      ...
      const [companiesRes, categoriesRes, documentRes, itemsRes] = await Promise.all([
        supabase.from('companies').select('*').eq('type', 'supplier').order('name'),
        supabase.from('product_categories').select('*').order('name'),
        supabase.from('uploaded_documents').select('*').eq('id', documentId).single(),
        supabase.from('document_items').select('*').eq('document_id', documentId).order('line_number'),
      ]);
      ...
      if (documentRes.data) {
        setFormData({
          company_id: documentRes.data.company_id || '',
          invoice_number: documentRes.data.invoice_number || '',
          invoice_date: documentRes.data.invoice_date || '',
          due_date: documentRes.data.due_date || '',
          notes: documentRes.data.notes || '',
        });
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Dokumentas sėkmingai išsaugotas!' });
      setTimeout(() => {
        onSaved();
      }, 1000);
    } catch (error: any) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full my-8">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center rounded-t-lg">
          <h2 className="text-2xl font-bold text-gray-900">Koreguoti dokumentą</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 max-h-[calc(100vh-200px)] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Tiekėjo informacija</h3>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Prekės / Paslaugos</h3>
                <button
                  type="button"
                  onClick={addItem}
              ...
              <div className="space-y-4">
                {items.map((item, index) => (
                  <div key={index} className="border border-gray-200 bg-white p-4 rounded-lg">
                    <div className="flex justify-between items-center mb-3">
                      <h4 className="font-medium text-gray-900">Prekė {index + 1}</h4>
                      {items.length > 1 && (
                        <button
                          type="button"
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-gray-700">
                  ...
                  <span className="font-semibold">{totalNoVat.toFixed(2)} EUR</span>
                </div>
                <div className="flex justify-between text-gray-700">
                  <span>PVM suma:</span>
                  <span className="font-semibold">{vatAmount.toFixed(2)} EUR</span>
                </div>
                <div className="flex justify-between text-xl font-bold text-gray-900 pt-2 border-t">
                  <span>Suma su PVM:</span>
                  <span>{totalWithVat.toFixed(2)} EUR</span>
                </div>
              </div>
            </div>
