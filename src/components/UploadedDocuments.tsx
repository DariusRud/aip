...
// Importai DocumentReview ir DocumentEdit praleisti, darome prielaidą, kad jie egzistuoja

type UploadedDocument = Database['public']['Tables']['uploaded_documents']['Row'] & {
  companies?: {
    name: string;
  } | null;
  company_id: string; // Pridedame, kad būtų prieinamas ir sąsajoje
};
...
// PAKEISTA: paleisti, kai pasikeičia statusFilter arba userCompanyId
  useEffect(() => {
    fetchDocuments();
...
          )
        `)
        // PRIDĖTAS PAGRINDINIS FILTRAS: Rodyti TIK šios įmonės dokumentus
...
        .eq('uploaded_document_id', documentId)
        // PRIDĖTAS FILTRAS: Rodyti tik šios įmonės dokumento prekes
        .eq('company_id', userCompanyId) 
...
        .delete()
        .eq('id', docId)
        .eq('company_id', userCompanyId); // APSAUGA: Trinti galima tik savo įmonės dokumentus
...
          sum_with_vat: document.total_amount,
          company_id: userCompanyId, // PRIDĖTAS: Perkelta sąskaita privalo turėti company_id
        })
        .select()
        .single();
...
        total_amount: item.total_amount,
      // Pakeičiame, kad item'ai irgi turėtų company_id, jei purchase_invoice_lines lentelė tai palaiko
...
        .eq('id', docId)
        .eq('company_id', userCompanyId); // APSAUGA: Atnaujinti galima tik savo dokumentus
...
      .select('*')
      .eq('uploaded_document_id', documentId)
      .eq('company_id', userCompanyId); // APSAUGA: Tik šios įmonės prekės
...
          documentId={selectedDocumentId}
          // PRIDĖTA: perduodame companyId redagavimui/išsaugojimui
...
        <DocumentReview
          documentId={selectedDocumentId}
          // PRIDĖTA: perduodame companyId peržiūrai/patvirtinimui
          userCompanyId={userCompanyId}
          onClose={() => {
            setSelectedDocumentId(null);
            setEditMode(false);
