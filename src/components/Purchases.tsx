...
  currency: string;
  notes: string | null;
  company_id: string; // PRIDĖTA: būtina sąskaitos atskyrimui
  created_at: string;
}
...
  category_id: string | null;
  company_id: string; // PRIDĖTA: būtina prekės atskyrimui
}

interface Category {
  id: string;
  name: string;
  // Darome prielaidą, kad kategorijos yra globalios arba filtruojamos pagal poreikį.
  // Jei jos yra specifinės, reikėtų pridėti company_id ir čia.
}

interface PurchasesProps {
  userRole: string;
  userCompanyId: string; // PRIDĖTAS NAUJAS PROP'as iš App.tsx
}

// PAKEISTA: priima userCompanyId
function Purchases({ userRole, userCompanyId }: PurchasesProps) { 
...
  // PAKEISTA: paleisti, kai pasikeičia userCompanyId
  useEffect(() => {
    fetchInvoices();
    fetchCategories();
  }, [userCompanyId]); 
...
        .from('invoices')
        .select('*')
        // PRIDĖTAS FILTRAS: Rodyti TIK šios įmonės sąskaitas
...
        .from('purchases')
        .select('*')
        // PRIDĖTAS FILTRAS: Rodyti tik prekes, priklausančias šios įmonės sąskaitoms
        .eq('invoice_id', invoiceId)
        .eq('company_id', userCompanyId) // PAPILDOMA APSAUGA: Tik šios įmonės prekės
...
        .select('id, name')
        // Jei kategorijos yra multi-tenant, čia taip pat reikėtų pridėti .eq('company_id', userCompanyId)
        .order('name');
...
    e.preventDefault();
    // PAKEISTA: Patikrinimas atsižvelgiant į App.tsx roles
...
        .insert([{
          ...newInvoice,
          company_id: userCompanyId, // PRIDĖTA: priskiriame įmonės ID
          created_by: user?.id,
        }]);
...
    e.preventDefault();
    // PAKEISTA: Patikrinimas atsižvelgiant į App.tsx roles
    if ((userRole.toLowerCase() !== 'admin' && userRole.toLowerCase() !== 'super admin') || !selectedInvoiceId) {
...
        .from('invoices')
        .delete()
        // PRIDĖTA APSAUGA: Trinti galima TIK savo įmonės sąskaitas
        .eq('id', id)
        .eq('company_id', userCompanyId); 
...
        .from('purchases')
        .delete()
        // PRIDĖTA APSAUGA: Trinti galima TIK savo įmonės prekes
        .eq('id', id)
        .eq('company_id', userCompanyId); 
...
  }
  ...
  // PAKEISTA: Bendras vaidmens tikrinimas visuose mygtukuose
...
            {/* ... sąskaitų filtravimas, jei reikalingas */}
          </div>
...
            </h2>
            {selectedInvoiceId && isAdminOrSuperAdmin && ( // PAKEISTA
              <button
...
      </div>
    ...
    {/* ... Modalo turinys (handleAddInvoice ir handleAddPurchase naudoja company_id) */}
    {/* (Modalo kodas praleistas dėl trumpumo) */}
    {showInvoiceModal && (
