import { useState, useMemo, useEffect } from 'react'; // PATAISYTA: Pašalintas nenaudojamas 'React'
...
    // const defaultCompany = currentUserRole === 'Super Admin' ? 'Dariaus Rudvalio IV' : DUMMY_USERS.find(u => u.company === DUMMY_USERS.find(du => du.role === 'Admin')?.company)?.company || INITIAL_COMPANY_NAMES[0]; // PATAISYTA: Nenaudojama
    
...
        
      _ const userToDelete = users.find(u => u.id === id);
...
                            >
                gali                 <i className="fas fa-trash text-base"></i>
{ ' ' }                       </button>
                        )}
                    </div>
                </td>
...
                  _     <table className="w-full min-w-[1000px] divide-y divide-slate-200">
...
SI                               <th className="text-left py-3 px-4 text-xs font-semibold text-slate-700 w-20">BŪSENA</th>
...
s                             {filteredUsers.map(renderUserRow)}
                            </tbody>
                        </table>
N                   </div>
                </div>
...
</i>                           className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 transition-colors"
                            >
                                Atšaukti
e                         </button>
                            <button
...
    // const isSuperAdminEdit = user && user.role === 'Super Admin'; // PATAISYTA: Nenaudojama
...
                    </button>
                </div>
              T   <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    
        _             <div>
...
                            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
ar                         {/* Super Admin rolę gali keisti tik pats Super Admin, ne Admin */}
...
s                           disabled={currentUserRole === 'Admin' && !!user} 
                  _       className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
...
                    V       className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 mr-3">
                            Atšaukti
            </i>         </button>
                        <button type="submit"
                            className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
Note                       {user ? 'Išsaugoti' : 'Pridėti'}
                        </button>
                    </div>
  V             </form>
            </div>
